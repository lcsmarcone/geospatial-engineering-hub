...
const ToptalBadge = () => {
  return (
    <motion.a
      href="https://www.toptal.com/developers/resume/lucas-marcone-dos-santos#M59mez"
      target="_blank"
      rel="noreferrer"
      aria-label="Hire Lucas Marcone on Toptal"
      initial={{ opacity: 0, scale: 0.92, y: 18 }}
      animate={{ opacity: 1, scale: 1, y: 0 }}
      transition={{ delay: 0.72, duration: 0.45 }}
      whileHover={{ y: -3, scale: 1.01 }}
      className="group relative z-10 block w-[176px] text-white outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-4 focus-visible:ring-offset-background sm:w-[190px] lg:w-[214px]"
      style={{ filter: 'drop-shadow(0 24px 42px hsl(var(--primary) / 0.22))' }}
    >
      <div
        className="bg-[#25a9ef] p-[5px] transition-transform duration-300 group-hover:-rotate-1"
        style={{ clipPath: hexagon }}
      >
        <div
          className="flex aspect-[1/1.18] flex-col items-center justify-center gap-2 bg-[linear-gradient(153deg,#0667ff_18%,#204ecf_42%,#0f256e_82%)] px-5 pb-7 pt-5 text-center ring-1 ring-white/15"
          style={{ clipPath: hexagon }}
        >
          <StarCrown />
          <h2 className="m-0 text-[17px] font-bold leading-none tracking-normal sm:text-[18px]">
            TOP 3% TALENT
          </h2>
          <div className="h-px w-24 bg-[#25a9ef]" />
          <span className="text-sm font-medium text-white/80">Vetted by</span>
          <ToptalLogo />
          <span className="mt-1 rounded-md bg-[#296bff] px-5 py-1.5 text-sm font-semibold text-white underline decoration-white/70 decoration-1 underline-offset-2 transition-colors group-hover:bg-[#25a9ef]">
            Hire me
          </span>
        </div>
      </div>
